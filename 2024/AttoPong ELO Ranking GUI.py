...
############### GUI #############################
#%%
listPlayers = tuple(Attopong.keys())
activePlayersBool = tuple(True for player in listPlayers) #all active players for now
...
myTip = Hovertip(entry_date,'Format is YearMonthDate (20230309)')
...
def update_ranking():
    ...
    if P1[0] != P2[0]:
        ...
        df =pd.DataFrame({'Date': [ match_date],
                        ...
                        'Score1': [P1[1]],
                        'Score2': [P2[1]]})
        df.to_csv('ScoreSheet\\2023.txt', mode='a', index=False,header=not os.path.exists('ScoreSheet\\2023.txt'),sep='\t')
        #Update results
...
def reloadlast_ranking():
    sound3()    
    with open("ScoreSheet\\2023.txt", "r") as f: # Open Scoresheet (read)
        datafile = f.read().splitlines() #Extract each line from file        
    with open("ScoreSheet\\2023.txt", "w") as f: # Open Scoresheet (write)
